...
node {
    ...
    println jwt_key_file 
        stage('Deploye Code') {
			...
			if (isUnix()) {
				rmsg = sh returnStdout: true, script: "sfdx force:source:deploy -p ./force-app/main/default  -u ${HUB_ORG}"
			}else{
			   rmsg = bat returnStdout: true, script: "sfdx force:mdapi:deploy -d ./force-app/main/default -w 5 -u ${HUB_ORG}"
			}
